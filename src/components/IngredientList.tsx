...
import type { AnalyzedIngredient } from "@/lib/analyzeProduct";

type IngredientListProps = {
  ingredients: AnalyzedIngredient[];
  onSelect: (item: AnalyzedIngredient) => void;
  emptyState?: {
    title: string;
    text: string;
  };
...
export default function IngredientList({
  ...
  const redCount = ingredients.filter((ingredient) => ingredient.level === "red").length;
    ...
    <section className="border-b border-[var(--line)] px-4 py-3.5">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className="text-[12px] text-[#617068]">{ingredients.length} items</span>
          {redCount > 0 ? (
            ...
            <div className="divide-y divide-[#eee4d5]">
              {ingredients.map((ingredient) => (
                <button
                  ...
                  className={`grid min-h-[2.5rem] w-full grid-cols-[auto_minmax(0,1fr)_auto_auto] items-center gap-1.5 px-1 py-2.5 text-left transition active:scale-[0.99] sm:gap-2 ${rowToneClasses[ingredient.level]}`}
                >
                  <ConcernDot level={ingredient.level} />
                  <span className="truncate text-[14px] font-medium text-[#1b2c24]">
                    {ingredient.name}
                  </span>
                  <StatusPill tone={ingredient.level} className="justify-self-end">
                    {ingredient.rowStatusLabel}
                  </StatusPill>
                  {ingredient.level !== "green" ? (
                    <RowActionText tone={ingredient.level} />
                  ) : (
                    <span />
                  )}
                </button>
